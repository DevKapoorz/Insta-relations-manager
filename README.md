...
Insta Relations Manager operates entirely under a **Local-First Privacy Architecture**:
- All operations (`JSZip` decompression, JSON parsing, set differences) run in your browser's local sandbox memory.
- No network requests containing your followers, followings, or username data are made.
- Optional anonymous analytics only measure aggregate usage counts (e.g. archive processed) without any personal data or usernames.
